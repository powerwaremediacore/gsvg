...
class GSvgTest.Suite : Object
{
  static int main (string args[])
  {
    ...
    Test.init (ref args);
    Test.add_func ("/gsvg/polygon/point",
    ()=>{
      var p1 = new GsPoint ();
      p1.parse ("1,2");
    ...
    Test.add_func ("/gsvg/polygon/points",
    ()=>{
    ...
    ()=>{
      try {
        ...
        assert ("<svg xmlns=\"http://www.w3.org/2000/svg\"><polygon points=\"1,2 4,6 7,9\"/></svg>" in svg.write_string ());
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1e+03 3e+02" width="10cm" height="3cm"><desc>Example tspan01 - using tspan to change visual attributes</desc><g font-family="Verdana" font-size="45"><text fill="blue" x="200" y="150">You are <tspan font-weight="bold" fill="red">not</tspan> a banana</text></g><polygon id="polygon" fill="none" stroke="blue" stroke-width="2" points="0,0 10,0 20,1 15,10 10,10"/></svg>""";
      ...
      var sh = svg.get_element_by_id ("polygon") as PolygonElement;
      assert (sh != null);
      assert (sh is PolygonElement);
      assert (sh.id == "polygon");
      message (sh.points.value);
      assert (sh.points.value == "0,0 10,0 20,1 15,10 10,10");
      assert (svg.polygons != null);
      message (svg.write_string ());
      message (svg.polygons_map.length.to_string ());
      assert (sh.node_name == "polygon");
      assert (svg.get_elements_by_tag_name ("polygon").length == 1);
      assert (svg.polygons is PolygonElementMap);
      assert (svg.polygons_map is GsPolygonElementMap);
      assert (svg.polygons_map is PolygonElementMap);
      assert (svg.polygons_map.items_type.is_a (typeof (PolygonElement)));
      assert (svg.polygons_map.length == 1);
      assert (svg.polygons.length == 1);
      assert (svg.polygons.get ("polygon") is PolygonElement);
    });
    return Test.run ();
  }
}
